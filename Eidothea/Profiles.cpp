...
// S O M E  N E E D  F U N C T I O N //

#define ranged_for(var, min, max, step) for(auto var = (min); var < (max); var += (step))

char path_to_calibration_distance_file[] = "XML_file/distantion.xml";
	...
	ElementInfo(const std::vector<cv::Point>& contour, cv::Mat& thresholded)
	{
		...
		for (int i = _upright.x; i < _upright.x + _upright.width; ++i)
		{
			...
			if (point_sum.size() > 0)
			{
				...
				medium_point = medium_point / (int)point_sum.size();
...
Profiles::Profiles(double dist_cam_to_laser)
{
...
void Profiles::Find_coeff(double center_y, double *D_real, 
						   float step,      char *path_to_folder)
{
	...
	{
		Find_medium_laser(cv::imread(fn[i]));
		double difference_y = abs(center_y - y_middle.at(y_middle.size() / 2));
...
void Profiles::calculate_m_b(double center_y,  double *D_real,
							  float step,	    char *path_to_folder)
{
	...
	if (fs.isOpened())	{
		...
		Find_coeff(center_y, D_real, step, path_to_folder);
	}
}
...
void Profiles::Find_HSV_Parameters(cv::Mat image)
{
	...
	for (;;)
	{
		...
		if ((difference_y > 0.08) & (hsv_parameters.v_min < 255))
		{
...
void Profiles::Find_medium_laser(cv::Mat image)
{
	cv::Mat imgHSV;
	...
	sort(new_point.begin(), new_point.end(), myobject);
	for (int i = 0; i < new_point.size(); i++)
	{
		y_middle.push_back(new_point.at(i).y);
	}
	//Отрисовка элементов
	//for (const ElementInfo& elin : elemInfos)
	//{
...
void Profiles::Calculate_coordinate(double center_y)
{
	if (m == 0 || b == 0) {
		std::cout << "Error: coeeficient m or b is not defined." << std::endl;
	}
	else {
		float n = y_middle.size();
		double pfc = 0;
		...
		for (int i = 0; i < n; i++)
		{
			...
			z.push_back(z_calc);
			//std::cout << "z_calc[" << i << "i]: " << z_calc << std::endl;
		};
	}
	MNK();
	///ПЕРЕДЕЛАТЬ
	float s = 0;
	float maxim = *max_element(z.begin(), z.end());
	s = (a_MNK*maxim + b_MNK) / 100;
	float x_s_step = s / y_middle.size();
	float x_left = -s / 2;
	float y_step = 693.5 * x_s_step;
	for (int k = 0; k < y_middle.size(); k++)
	{
		x.push_back(x_left);
		x_left += x_s_step;
		//std::cout << "X: " << x_left << std::endl;
	}
	for (int i = 0; i < y_middle.size(); i++)
	{
		y.push_back((y_step / 2 - (y_middle.at(i) * x_s_step)));
	}
	y_middle.clear();
	/// КОНЕЦ ПЕРЕДЕЛАТЬ
}

void Profiles::Safe_dots(std::vector<cv::Point3d>&	cloud,
	std::ofstream&				out,
	const char					*name)
{
	for (int i = 0; i < x.size(); i++)
...
double Profiles::Scan_frame(cv::Mat image, double center_y)
{
	clock_t start = clock();
	Find_medium_laser(image);
	Calculate_coordinate(center_y);
	clock_t end = clock();
	return (double)(end - start) / 1000;
}

void Profiles::MNK() //Переделать
{
	std::string line;
	double a, b;
	...
	std::ifstream fileMNK("dataMNK.csv");
...
//void Profiles::create_profiles(std::vector<cv::Point3d>& cloud,
...
//
//	a = new double*[rowAUV];
//	a[0] = new double[colAUV];
//	a[1] = new double[colAUV];
//	a[2] = new double[colAUV];
//
//	a[0][0] = cos(qsi)*cos(theta);
//	a[0][1] = -sin(qsi)*cos(phi) + cos(qsi)*sin(theta)*sin(phi);
//	a[0][2] = sin(qsi)*sin(phi) + cos(qsi)*cos(phi)*sin(theta);
//
//	a[1][0] = sin(qsi)*cos(theta);
//	a[1][1] = cos(qsi)*cos(phi) + sin(qsi)*sin(theta)*sin(qsi);
//	a[1][2] = -cos(qsi)*sin(phi) + sin(theta)*sin(qsi)*cos(phi);
//
//	a[2][0] = -sin(theta);
//	a[2][1] = cos(theta)*sin(phi);
//	a[2][2] = cos(theta)*cos(phi);
//
//	std::vector<double> vect;
//	vect.resize(3);
//
//	std::vector<double> out;// выходной вектор
//	out.resize(3);
//
//	for (int i = 0; i < x.size(); i++)
//	{
//		vect.at(0) = x.at(i), vect.at(1) = y.at(i), vect.at(2) = z.at(i);
//		for (int ix = 0; ix < rowAUV; ix++)
//		{
//			out.at(ix) = 0;
//			for (int jx = 0; jx < colAUV; jx++)
//				out.at(ix) += a[ix][jx] * vect.at(jx);
//		}
//		cloud.push_back(cv::Point3d(out.at(0) + X, out.at(1) + Y, out.at(2) + Z));
//	}
...
double Profiles::CalcDist(std::vector<double> p1, std::vector<double> p2) 
{
	return sqrt(pow(p1[0] - p2[0], 2) + pow(p1[1] - p2[1], 2) + pow(p1[2] - p2[2], 2));
}

void Profiles::triangul()
{
	int i = 1;
	double eps = 1.4;
	double maxDist = 10;
...
void Profiles::safe_cloud(std::ofstream &out, const char *name)
{
...
void Profiles::safe(std::ofstream &out, const char *name)
{
	std::ofstream	outData;
	int				count = 1;
